...
class ThreaManger:
    def __init__(self):
        self.num = settings.thread_num
        self.queue = queue.Queue()
        self.thredobj = self._init_thred()

    def _init_thred(self):
        for i in range(self.num):
            t = thread(self.queue)
            t.start()

    def doing(self,func,*args):
...
class thread(Thread):
    ...
    def run(self):
        while True:
            self.func,self.args = self.queue.get()
            self.func(self.args)
            self.queue.task_done()

# 服务器对象
class Server(ThreaManger):
    MAX_PAKET = 8192
    def __init__(self):
        super().__init__()
